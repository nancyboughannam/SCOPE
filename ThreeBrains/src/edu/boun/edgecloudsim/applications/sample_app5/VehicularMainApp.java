...
public class VehicularMainApp {
    ...
    /**
     * Main entry point for the vehicular edge computing simulation. Configures
     * and runs simulation scenarios with different policies and device counts.
     *
     * @param args command line arguments: [configFile, edgeDevicesFile,
     * applicationsFile, outputFolder, iterationNumber]
     */
    // ===== FIXED RANDOM SEED CHANGE START =====
    private static final long BASE_SIMULATION_SEED = 20260828L;
// ===== FIXED RANDOM SEED CHANGE END =====
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 5) {
            ...
            configFile = "ThreeBrains/scripts/sample_app5/config/" + configName + "_config.properties";
            applicationsFile = "ThreeBrains/scripts/sample_app5/config/applications.xml";
            edgeDevicesFile = "ThreeBrains/scripts/sample_app5/config/edge_devices.xml";
            outputFolder = "ThreeBrains/sim_results/ite" + iterationNumber;
        }

        // ===== SCOPE DATA COLLECTION CHANGE START: unique simulation run ID =====
        // Used to group candidate-training rows by independent simulation run.
        // A command-line -Dscope.runId=... value takes precedence when supplied.
        if (System.getProperty("scope.runId") == null) {
            System.setProperty("scope.runId", "iteration_" + iterationNumber);
        }
        // ===== SCOPE DATA COLLECTION CHANGE END =====
// ===== ABLATION RUN IDENTITY CHANGE START =====
// Sent to both optimizer services so every decision row can be
// joined unambiguously with this simulator iteration.
        System.setProperty(
                "scope.iterationNumber",
                Integer.toString(iterationNumber)
        );
        ...
        for (int i = SS.getMinNumOfMobileDev(); i <= SS.getMaxNumOfMobileDev(); i += SS.getMobileDevCounterSize()) {
            for (int s = 0; s < SS.getSimulationScenarios().length; s++) {
                for (int p = 0; p < SS.getOrchestratorPolicies().length; p++) {
                    ...
                    long simulationSeed
                            = BASE_SIMULATION_SEED + iterationNumber;

                    SimUtils.setSeed(simulationSeed);

                    System.setProperty(
                            "scope.simSeed",
                            Long.toString(simulationSeed)
                    );
